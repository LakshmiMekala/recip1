	...
	function multi_os ()
	{
		...
		if [ -n "${TRAVIS_TAG}" ]; then
			echo Creating build with release version:"${TRAVIS_TAG}";
				if [ -d "${TRAVIS_REPO_SLUG}-${TRAVIS_TAG}" ]; then			
		...
		if [ -z "${TRAVIS_TAG}" ]; then
			echo Creating build with travis build number:"${TRAVIS_BUILD_NUMBER}" ;
				if [ -d "${TRAVIS_REPO_SLUG}-${TRAVIS_BUILD_NUMBER}" ]; then			
					cd "${TRAVIS_REPO_SLUG}-${TRAVIS_BUILD_NUMBER}";
					create_gateway;
				else
					mkdir "${TRAVIS_REPO_SLUG}-${TRAVIS_BUILD_NUMBER}";
					cd "${TRAVIS_REPO_SLUG}-${TRAVIS_BUILD_NUMBER}";
					create_gateway;				
				fi
